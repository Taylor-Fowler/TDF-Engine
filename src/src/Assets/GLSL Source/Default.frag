...
void main(void)
{
	...
	FinalColour = mix(FinalColour * texture(texture0, vertexOut.TextureCoordinate.st),
					texture(reflectionCubeMap, vertexOut.TextureCoordinateCubeMap), reflectivityMaterial);

	// float shadow = 1.0;
	// if(Result.ShadowCoordinate.w > 0)
	// {
	//   shadow = 0.5 + 0.5 * textureProj(shadowMap, vertexOut.ShadowCoordinate);
	// }
	// FinalColour *= shadow;
}
