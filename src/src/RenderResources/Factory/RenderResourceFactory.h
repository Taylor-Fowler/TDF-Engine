...
public:
	RenderResourceFactory(RenderLoop *const renderLoop, FileReadWrite *const fileRW, ResourceManager *const resourceManager, Loader3D *const loader3D) 
	...
	virtual std::shared_ptr<Texture> CreateStaticTexture(const std::string& name) const = 0;
	...
	virtual Texture* CreateDynamicTexture(const std::string& name) const = 0;
	...
	virtual std::shared_ptr<Shader> CreateShader(const std::string& name) const = 0;
